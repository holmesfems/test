...
def load(filename,comment='#',delim='\t '):
    ...
    for line in f:
        line=line.strip()
        if line.find(comment)==0 or line=='':
            continue
        tmp=dds.split(line)
        res.append(tmp)
    f.close()
...
def loadnp(filename,comment='#',delim='\t ',labelmark='!'):
    tmp=load(filename,comment,delim)
    Xa=[]
    Yas=[]
...
def doplot(plots,option):
    ars = loadnp(plots)
    plt.figure(figsize=(4,3),dpi=int(option['dpi']))
    plt.subplot(111)
